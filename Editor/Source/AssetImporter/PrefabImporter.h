...
public:
    ...
    inline static Ref<PrefabAsset> CreatePrefabAsset(const std::string& path, Scene& scene, entt::entity& rootEntity)
    {
...
private:
    static void SerializeEntityRecursively(entt::entity& e, Scene& scene, std::vector<uint8_t>& buffer)
    {
        // serialize component count
        unsigned int componentCount = GetComponentCount(scene, e);
        buffer.push_back(static_cast<uint8_t>(componentCount));
